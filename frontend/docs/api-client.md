...
继续复核 URL、状态、MIME、15 MiB 上限和文件签名，再转换为与本地选图相同的待上传图片草稿。查询阶段不创建
WineStock 文件对象，最终保存仍统一调用图片上传接口。
Core 只根据立创查询响应字段按 source、首张 breviary、`bigImageUrl` 的顺序选择一个地址，不额外下载或探测图片。
图片请求只在用户确认覆盖后发起；图片缺失或读取失败时生成带客编的默认占位图，不回滚已经应用的资料、模板属性和参考价格；
分类、单位和库存设置保持不变。
