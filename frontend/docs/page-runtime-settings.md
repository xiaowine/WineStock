...
- **设置未完成**时其它路由强制进入本页：`unconfigured` / `invalid` / 无有效 `apiBaseUrl`，或 Shell `initialized === false`。`returnTo` 保留原目标。
...
- Web 未配置/无效，或 Shell `initialized=false` 时必须进入本页；用户确认后的已完成配置可直接认证/业务。
