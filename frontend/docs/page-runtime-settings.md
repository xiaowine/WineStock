...
- **设置未完成**时其它路由强制进入本页：`unconfigured` / `invalid` / 无有效 `apiBaseUrl`，或 Shell `createdDefault === true`（自动默认未确认）。`returnTo` 保留原目标。
- **设置已完成**（可 HTTP 且非 `createdDefault`）时不强制进入；可从账户弹层、认证页、服务不可用覆盖层等侧入口打开。
- 服务不可用覆盖层不得遮挡本页。
- 不删除 Shell/Web 对默认 `apiBaseUrl` 的注入；漏斗用 `createdDefault` 要求用户确认（方案 B）。

## 启动漏斗中的位置

界面顺序为：**运行设置 → 统一认证入口 `/auth` → 注册/登录 → 业务**。

- **`createdDefault` 仅由「保存设置」清除**：自动默认时即使表单未改也可点保存；未保存前不展示「继续」。
...
- 本机运行：使用 `127.0.0.1`，端口由 Shell 自动选择并持久化实际值，应用启动时必须自动运行本地服务；页面不展示端口或监听地址输入框。
...
- Web 未配置/无效，或 Shell `createdDefault` 自动默认时必须进入本页；用户确认后的已完成配置可直接认证/业务。
