...
本文记录运行模式 Dialog 的当前实现边界。它是 Desktop、Android 和 Web fallback 共用的唯一运行配置 UI，正常已进入应用时由 AppShell 以 Dialog 打开；平台 Shell 不提供原生设置窗口、Activity 或对话框。
...
本机静默免登录会话（`localSilentAuthActive`）下把运行模式切到「共享服务」提交时：
...
界面顺序为：**运行模式 → 统一认证入口 `/auth` → 注册/登录 → 业务**。
...
- 本机自用：使用 `127.0.0.1`，首次 apply 或端口冲突时由 Shell 自动选择并持久化实际值；已有配置的应用启动时自动运行本地服务；页面不展示端口或监听地址输入框。
- 连接远端：编辑完整 HTTP/HTTPS API 根地址，本设备不管理本地服务。
- 共享服务：本机启动服务并允许其它设备连接；固定端口和监听地址收纳在默认收起的“高级设置”中，是否可选由 Shell capability 决定。
- Android 上“共享服务”会明确显示当前只能使用本机 `127.0.0.1` 的原因，不会伪装成普通禁用态。
- 纯网页端（快照 `platform === "web"`）只有连接远端一种能力：“本机自用”和“共享服务”均禁用并显示“浏览器无法在本机启动服务”的原因；本机类持久配置在进入页面时被动纠正为远端草稿（须保存才生效）。与初始化向导在 web 下跳过使用方式页的语义一致（见 `docs/implementation-notes/first-run-setup-wizard.md`）。
...
- “当前端口已允许局域网访问”只在当前已应用模式为 `server-mode` 时显示；本机自用和连接远端不展示这条防火墙状态。
- 局域网地址入口只展示 Shell 返回的真实地址；没有可用地址时不生成占位 URL，也不在运行模式 Dialog 中额外提示。
...
- 没有 LAN URL 与防火墙未放行是两类不同问题：前者提示网络适配器/绑定地址，后者提示规则授权或系统策略；
  不能只用一条混合错误文案。
- 连接远端时不提供单独的“测试连接”按钮；运行设置页点击“测试并保存”或初始化向导点击“完成”后，
  都会先检查远端 `/api/health`，测试成功才保存地址，暂时不可连接则保留草稿并提示错误。
