...
当前已经完成页面框架、响应式 Shell、API client、统一会话恢复、注册/登录/修改密码、登出、鉴权与强制改密守卫、用户管理、库存总览、物品管理、库位主数据管理、分类与模板、替代关系全局治理、操作日志，以及入库/出库单物品选择后立即配置的单物品串行工作台；其它库存业务页面按路由与 API 实现状态继续推进。
...
- `page-events.md`：操作日志页面的服务端筛选、三段式列表、历史 JSON 详情、分页、响应式和实施验收设计。
...
- [出入库选择物品即配置明细整改方案](implementation-notes/inbound-outbound-item-operation-flow-remediation.md)：工作流、状态模型、兼容策略和验收矩阵。
