...
当前已经完成页面框架、响应式 Shell、API client、统一会话恢复、注册/登录/修改密码、登出、鉴权与强制改密守卫、用户管理、库存总览、物品管理、库位主数据管理、分类与模板、替代关系全局治理、审计日志，以及入库/出库单物品选择后立即配置的单物品串行工作台；其它库存业务页面按路由与 API 实现状态继续推进。
