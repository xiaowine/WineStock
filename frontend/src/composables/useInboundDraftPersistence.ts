...
import {
  clearInboundDraftImages,
  readInboundDraftImage,
  replaceInboundDraftImages,
} from "../storage/inboundDraftImageStore";
...
export function useInboundDraftPersistence(
  ...
  async function restore(): Promise<boolean> {
    ...
    try {
      ...
      if (![4, 5].includes(draft.version) || !Array.isArray(draft.lines))
      ...
      lines.value = await Promise.all(
        draft.lines.map(async (line) => ({
          ...
          extAttributes: await restoreAttributes(line.lineId, line.extAttributes),
          template: null,
          templateId: line.templateId,
          recommendedTemplateId:
            line.recommendedTemplateId ?? line.item.recommended_inbound_template_id ?? null,
...
function isPersistedFileValue(value: unknown): value is PersistedFileValue {
  return (
    ...
    "kind" in value &&
    value.kind === "file" &&
    (("fileId" in value && typeof value.fileId === "number") ||
      ("localKey" in value && typeof value.localKey === "string")) &&
    "name" in value &&
