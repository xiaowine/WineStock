...
export function useSubstituteNetworkLayout() {
  ...
  function mergeGraph(nodes: readonly SubstituteNetworkNode[], edges: readonly SubstituteNetworkEdge[], restartLayout = true): void {
    ...
    activeNodes = nodes
    activeEdges = edges.filter((edge) => activeIds.has(edge.sourceId) && activeIds.has(edge.targetId))
  ...
  function reset(): void {
    ...
    for (const node of activeNodes) next[node.id] = deterministicPosition(node.id, activeNodes.length)
    positions.value = next
...
function deterministicNeighborPosition(nodeId: number, neighbors: readonly SubstituteNetworkPosition[]): SubstituteNetworkPosition {
...
function appendNeighbor(map: Map<number, SubstituteNetworkPosition[]>, nodeId: number, position: SubstituteNetworkPosition): void {
