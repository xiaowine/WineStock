...
export function useInboundItemCatalog(errorMessage: (error: unknown) => string) {
  ...
  const resultLabel = computed(() => loadingItems.value && items.value.length === 0 ? '正在加载结果' : `共 ${totalItems.value} 个物品`)
  ...
  async function loadNext(): Promise<void> {
    ...
    try {
      ...
      if (generation === requestGeneration && !(error instanceof DOMException && error.name === 'AbortError')) {
  ...
  function handleScroll(): void {
    ...
    if (element && element.scrollHeight - element.scrollTop - element.clientHeight < 160) void loadNext()
  }
