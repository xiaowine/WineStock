...
import type { InboundTemplateResponse, InboundTemplateWriteRequest } from '../../api/inboundTemplates'
import type { TemplateFieldRequest, TemplateFieldResponse, TemplateFieldType } from '../../api/templateFields'
...
export function createTemplateDraft(
  ...
  return {
    ...
    defaultInboundTemplateId: kind === 'item' && 'default_inbound_template_id' in template
...
export function validateTemplateDraft(draft: TemplateDraft, kind: AttributeTemplateKind): TemplateDraftValidation {
  ...
  draft.fields.forEach((field, index) => {
    ...
    if (field.fieldType === 'select') validateStringOptions(field.options, prefix, 'options', 128, 128, errors)
    if (kind === 'item') {
      if (field.catalogVisible) catalogVisibleCount += 1
      validateUnit(field, prefix, errors)
    }
    if (firstFieldIndex === null && Object.keys(errors).some((key) => key.startsWith(`${prefix}.`))) {
...
export function clearIncompatibleFieldData(field: TemplateFieldDraft, nextType: TemplateFieldType): void {
  field.fieldType = nextType
  if (nextType !== 'select') field.options = []
  if (nextType === 'file') field.defaultValue = ''
  if (nextType === 'boolean' && !['true', 'false'].includes(field.defaultValue)) field.defaultValue = ''
...
export function countFieldTypes(fields: readonly TemplateFieldResponse[]): string {
  ...
  const parts = Array.from(counts.entries()).map(([type, count]) => `${fieldTypeLabel(type)} ${count}`)
  return parts.length > 3 ? `${parts.slice(0, 3).join(' · ')} · 另 ${parts.length - 3} 类` : parts.join(' · ')
}

function responseFieldToDraft(field: TemplateFieldResponse, kind: AttributeTemplateKind): TemplateFieldDraft {
...
function buildUnitRule(field: TemplateFieldDraft): ItemAttributeUnitRule {
  if (field.unitMode === 'fixed') return { mode: 'fixed', value: field.unitValue.trim(), options: null }
  if (field.unitMode === 'select') return { mode: 'select', value: null, options: field.unitOptions.map((option) => option.trim()) }
  return { mode: 'none', value: null, options: null }
}

function validateDefaultValue(field: TemplateFieldDraft, prefix: string, errors: Record<string, string>): void {
  const value = field.defaultValue.trim()
  if (value.length > 256) errors[`${prefix}.default_value`] = '默认值不能超过 256 个字符'
  if (!value) return
  if (field.fieldType === 'number' && !Number.isFinite(Number(value))) errors[`${prefix}.default_value`] = '请输入有效数字'
  if (field.fieldType === 'boolean' && !['true', 'false'].includes(value)) errors[`${prefix}.default_value`] = '布尔默认值只能是是或否'
  if (field.fieldType === 'url' && !isHttpUrl(value)) errors[`${prefix}.default_value`] = '链接必须使用 HTTP 或 HTTPS'
  if (field.fieldType === 'date' && !isCalendarDate(value)) errors[`${prefix}.default_value`] = '请输入有效的 YYYY-MM-DD 日期'
...
function validateUnit(field: TemplateFieldDraft, prefix: string, errors: Record<string, string>): void {
