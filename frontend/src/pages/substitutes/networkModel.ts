...
export function buildSubstituteNetwork(relations: readonly SubstituteRelationResponse[]): SubstituteNetworkGraph {
  ...
  for (const relation of relations) {
    ...
    ensureNode(mutableNodes, relation.substitute_item_id, relation.substitute_item_name, relation.substitute_item_sku)
  ...
  const nodes = Array.from(mutableNodes.values())
    .map((node): SubstituteNetworkNode => ({ ...node, degree: node.incomingCount + node.outgoingCount }))
    .sort(compareNetworkNodes)
  const edges = Array.from(edgeById.values()).sort(
    (left, right) => left.sourceId - right.sourceId || left.priority - right.priority || left.targetId - right.targetId,
  )
...
/** 返回当前节点和一跳上下游组成的节点集合。 */
export function getDirectRelationNodeIds(graph: SubstituteNetworkGraph, nodeId: number): Set<number> {
...
export function getReachableDownstreamNodeIds(graph: SubstituteNetworkGraph, nodeId: number): Set<number> {
...
export function searchSubstituteNetworkNodes(
  ...
  if (!keyword) return []
  return graph.nodes
    .filter((node) => node.name.toLocaleLowerCase('zh-CN').includes(keyword) || node.sku.toLocaleLowerCase('zh-CN').includes(keyword))
    .sort((left, right) => right.degree - left.degree || compareNetworkNodes(left, right))
    .slice(0, limit)
}

/** 根据规模和中心节点选择当前允许渲染的节点集合。 */
export function getRenderableNodeIds(graph: SubstituteNetworkGraph, selectedId: number | null): Set<number> {
  ...
  return new Set(graph.nodes.slice().sort((left, right) => right.degree - left.degree || left.id - right.id).slice(0, 80).map((node) => node.id))
}
...
function compareNetworkNodes(left: Pick<SubstituteNetworkNode, 'name' | 'id'>, right: Pick<SubstituteNetworkNode, 'name' | 'id'>): number {
