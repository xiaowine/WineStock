...
/** 按主物品聚合关系，并稳定保持优先级顺序。 */
export function groupSubstituteRelations(
  ...
  return Array.from(groups.values())
    .map((items) => {
      const sorted = items
        ...
        .sort(
          (left, right) =>
            left.priority - right.priority || left.substitute_item_id - right.substitute_item_id,
        );
    ...
    .sort(
      (left, right) =>
        left.itemName.localeCompare(right.itemName, "zh-CN") || left.itemId - right.itemId,
    );
}
