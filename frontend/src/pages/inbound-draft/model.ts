...
export function validQuantity(value: number): boolean { return Number.isFinite(value) && value > 0 }
export function validUnitPrice(value: number): boolean { return Number.isFinite(value) && value >= 0 }
export function positiveNumber(value: number): number { return Number.isFinite(value) && value > 0 ? value : 0 }
export function lineSubtotal(line: InboundDraftLine): number { return positiveNumber(line.quantity) * (validUnitPrice(line.unitPrice) ? line.unitPrice : 0) }

/** 按服务端当前模板规则校验单个字段，并返回可直接展示的中文原因。 */
export function templateFieldError(line: InboundDraftLine, field: TemplateFieldResponse): string | null {
  const value = line.extAttributes[field.field_name]
  const empty = value === undefined || value === null || value === ''
  if (empty) return field.required ? '此项为必填属性' : null
  if (field.field_type === 'number') return Number.isFinite(Number(value)) ? null : '请输入有效数字'
  if (field.field_type === 'url') return validHttpUrl(String(value)) ? null : '请输入 HTTP 或 HTTPS 地址'
  if (field.field_type === 'select') return field.options?.includes(String(value)) ? null : '请选择有效候选值'
...
export function buildInboundRequest(
  ...
  return {
    ...
    items: lines.map((line) => ({
      ...
      expires_at: line.expiresAt || undefined, inbound_template_id: line.templateId ?? undefined,
...
function normalizedAttributes(line: InboundDraftLine): Record<string, string | number | boolean | FileAttributeReference> | undefined {
  ...
  for (const field of line.template.fields) {
    ...
    } else attributes[field.field_name] = field.field_type === 'number' ? Number(value) : value as string | number | boolean
  }
