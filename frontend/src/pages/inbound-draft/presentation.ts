...
export function inboundSubmitErrorMessage(error: unknown): { title: string; detail?: string } {
  ...
  if (error instanceof ApiNetworkError) return { title: '无法连接到 WineStock 服务', detail: '草稿仍保存在本机，请恢复连接后重试。' }
  if (error instanceof ApiResponseError) return { title: '服务响应版本不匹配', detail: '请确认前后端版本一致后重试。' }
  if (error instanceof ApiConfigurationError) return { title: '服务地址配置无效', detail: error.message }
  return { title: '提交入库单失败', detail: '草稿仍保存在本机，请稍后重试。' }
}

export function itemErrorMessage(error: unknown, fallback = '加载物品失败'): string {
  if (error instanceof ApiError) return error.code === 'permission_denied' ? '当前账号没有执行此操作的权限' : error.message
