...
<template>
  <div ref="host" class="substitute-network-canvas" :class="{ 'substitute-network-canvas--settling': settling }">
    ...
    >
      <defs>
        ...
        <marker :id="activeMarkerId" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto" markerUnits="strokeWidth">
          <path d="M0,0 L8,4 L0,8 Z" />
        </marker>
      </defs>
        ...
        <g class="substitute-network-canvas__nodes">
          ...
          >
            ...
            <rect
              ...
              rx="5"
            />
            <text class="substitute-network-node__name" text-anchor="middle" :y="nodeRadius(node) + 21">
              {{ truncate(node.name, compactLabels ? 8 : 12) }}
            </text>
            <text v-if="showSkuLabels" class="substitute-network-node__sku" text-anchor="middle" :y="nodeRadius(node) + 34">
    ...
    <span v-if="showSettling" class="substitute-network-canvas__status" role="status">正在整理关系网络…</span>
  </div>
</template>
...
import type { SubstituteNetworkEdge, SubstituteNetworkNode } from '../../pages/substitutes/networkModel'
...
const { positions, settling, mergeGraph, reset: resetLayoutPositions, moveNode, releaseNode, stop } = useSubstituteNetworkLayout()
const showSettling = useStablePendingIndicator(settling, { showDelayMs: 200, minimumVisibleMs: 350 })
...
let dragMoved = false
let panStart: { pointerId: number; x: number; y: number; offsetX: number; offsetY: number } | null = null
let pinchStart: { distance: number; scale: number; centerX: number; centerY: number; offsetX: number; offsetY: number } | null = null
let hasAppliedInitialView = false
let hasAutoFitAfterLayout = false
let userAdjustedView = false

const contentTransform = computed(() => `translate(${viewState.value.offsetX} ${viewState.value.offsetY}) scale(${viewState.value.scale})`)
...
const showSkuLabels = computed(() => !compactLabels.value && viewState.value.scale >= 0.7 && props.nodes.length <= 180)
...
watch(
  ...
  async () => {
    ...
    if (focusedNodeId.value === null || !props.nodes.some((node) => node.id === focusedNodeId.value)) focusedNodeId.value = props.nodes[0]?.id ?? null
    await nextTick()
...
onMounted(() => {
  resizeObserver = new ResizeObserver(([entry]) => {
    ...
    const changed = Math.abs(width - canvasSize.value.width) > 8 || Math.abs(height - canvasSize.value.height) > 8
...
function fit(): void {
  ...
  const scale = clamp(Math.min(canvasSize.value.width / contentWidth, canvasSize.value.height / contentHeight) * 0.92, 0.45, 1.45)
  setViewState({
    scale,
...
function handlePointerMove(event: PointerEvent): void {
  ...
  if (panStart?.pointerId === event.pointerId) {
    ...
    setViewState({ ...viewState.value, offsetX: panStart.offsetX + deltaX, offsetY: panStart.offsetY + deltaY })
  }
}
...
function isEdgeActive(edge: SubstituteNetworkEdge): boolean {
  return props.selectedId !== null && (edge.sourceId === props.selectedId || edge.targetId === props.selectedId)
}
...
function edgeCurve(edge: SubstituteNetworkEdge, deltaX: number, deltaY: number, distance: number): { x: number; y: number } {
  const reverseExists = props.edges.some((candidate) => candidate.sourceId === edge.targetId && candidate.targetId === edge.sourceId)
