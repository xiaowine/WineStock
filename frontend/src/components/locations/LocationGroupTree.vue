...
<template>
  <div class="location-group-tree" role="tree" aria-label="库位分组">
    <div class="location-group-tree__row location-group-tree__row--all" role="treeitem" :aria-selected="selectedGroupId === null">
    ...
    >
      <button
        ...
        :aria-label="expandedGroupIds.includes(row.node.id) ? `收起 ${row.node.name}` : `展开 ${row.node.name}`"
        @click="emit('toggle', row.node.id)"
      >
        <svg viewBox="0 0 16 16" aria-hidden="true" :class="{ 'is-expanded': expandedGroupIds.includes(row.node.id) }">
          <path d="m6 3 5 5-5 5" />
        </svg>
      </button>
      ...
      <span v-if="canManage" class="location-group-tree__actions">
        <button
          ...
          :aria-label="row.depth + 1 >= MAX_LOCATION_GROUP_DEPTH ? `${row.node.name} 已达到 10 层上限` : `在 ${row.node.name} 下新建分组`"
          :disabled="row.depth + 1 >= MAX_LOCATION_GROUP_DEPTH"
          @click="emit('create-child', row.node)"
        >
          <svg viewBox="0 0 20 20" aria-hidden="true"><path d="M10 4v12M4 10h12" /></svg>
        </button>
        <button type="button" title="编辑分组" :aria-label="`编辑分组 ${row.node.name}`" @click="emit('edit', row.node)">
          <svg viewBox="0 0 20 20" aria-hidden="true"><path d="m4 14-.5 2.5L6 16l8.5-8.5-2-2L4 14Z"/><path d="m11.5 6.5 2 2"/></svg>
        </button>
        <button type="button" title="删除分组" :aria-label="`删除分组 ${row.node.name}`" @click="emit('delete', row.node)">
