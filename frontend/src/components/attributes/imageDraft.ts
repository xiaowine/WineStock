...
const solidColorPalette = ['#d97757', '#d59b45', '#86a45d', '#4f9b83', '#4f91aa', '#657fba', '#8174b2', '#ad6f92']
...
export async function createSolidColorImage(color: string, size = 512): Promise<File> {
  ...
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((value) => value ? resolve(value) : reject(new Error('纯色图片生成失败')), 'image/png')
  })
  return new File([blob], `solid-${color.replace('#', '').toLowerCase()}.png`, { type: 'image/png' })
}

/** 从受控色板随机生成默认物品主图草稿。 */
export async function createRandomSolidColorImageDraft(): Promise<ImageDraftValue> {
  const color = solidColorPalette[Math.floor(Math.random() * solidColorPalette.length)] ?? '#657fba'
