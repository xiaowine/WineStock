  ...
  >
    <template v-if="event" #context>
      <dl class="event-detail-context">
        <div><dt>事件</dt><dd>#{{ event.id }}</dd></div>
        <div><dt>操作时间</dt><dd>{{ formatLocalTimestamp(event.timestamp) }}</dd></div>
        <div><dt>操作人</dt><dd>{{ actorLabel(event) }}</dd></div>
        <div><dt>业务对象</dt><dd>{{ entityTargetLabel(event) }}</dd></div>
      </dl>
    </template>
    ...
    <div v-if="event" class="event-detail">
      <section class="event-detail__section">
        ...
        <dl class="event-detail__metadata">
          <div><dt>原始 UTC</dt><dd>{{ event.timestamp }}</dd></div>
          <div><dt>实体类型</dt><dd><code>{{ event.entity_type }}</code></dd></div>
          <div><dt>动作</dt><dd><code>{{ event.action }}</code></dd></div>
          <div><dt>用户 ID</dt><dd>{{ event.user_id === null ? '无记录' : `#${event.user_id}` }}</dd></div>
        </dl>
      </section>
      ...
      <section v-if="permissionChanges" class="event-detail__section">
        ...
        <div class="event-permission-diff">
          <div>
            ...
            <ul v-if="permissionChanges.added.length"><li v-for="permission in permissionChanges.added" :key="permission"><code>{{ permission }}</code></li></ul>
            <span v-else>无新增权限</span>
          </div>
          <div>
            <strong>移除权限 · {{ permissionChanges.removed.length }}</strong>
            <ul v-if="permissionChanges.removed.length"><li v-for="permission in permissionChanges.removed" :key="permission"><code>{{ permission }}</code></li></ul>
      ...
      <section v-if="diffRows.length" class="event-detail__section">
        ...
        <div class="event-diff-table" role="table" aria-label="审计字段变化">
          <div class="event-diff-table__head" role="row">
            <span role="columnheader">字段</span><span role="columnheader">修改前</span><span role="columnheader">修改后</span>
          </div>
      ...
      <section v-if="previousSnapshot.length" class="event-detail__section">
        ...
        <dl class="event-detail__entries">
          <div v-for="entry in previousSnapshot" :key="entry.key"><dt>{{ entry.label }}</dt><dd>{{ formatJsonValue(entry.value) }}</dd></div>
        </dl>
      </section>
      ...
      <section v-if="detailEntries.length" class="event-detail__section">
        <h3>结构化详情</h3>
        <dl class="event-detail__entries">
          <div v-for="entry in detailEntries" :key="entry.key"><dt>{{ entry.label }}</dt><dd>{{ formatJsonValue(entry.value) }}</dd></div>
        </dl>
      </section>
    ...
    <template #actions>
      ...
      <button v-if="event?.entity_id !== null" class="primary-button" type="button" @click="emitRelated">查看相关事件</button>
    </template>
  </ModalDialog>
</template>
...
import {
  eventDetailEntries, eventDiffRows, eventPermissionChanges, eventPreviousSnapshot,
  formatJsonValue, safeJsonStringify,
...
const dialogTitle = computed(() => props.event
...
const permissionChanges = computed(() => props.event ? eventPermissionChanges(props.event.details) : null)
const previousSnapshot = computed(() => props.event ? eventPreviousSnapshot(props.event.details) : [])
