...
<template>
  ...
  >
    <form v-if="!readOnly" :id="formId" class="template-editor" novalidate @submit.prevent="submit">
      <section class="template-editor__basics" aria-label="模板基础信息">
        ...
        <div v-if="kind === 'item'" class="template-editor__default-template">
          ...
          >
            ...
            <option v-for="option in inboundTemplates" :key="option.id" :value="option.id">{{ option.name }}</option>
          </FormSelect>
        </div>
      </section>
      ...
      <section class="template-editor__fields" aria-labelledby="template-fields-heading">
        <header class="template-editor__fields-toolbar">
          ...
          <button class="secondary-button" type="button" :disabled="submitting || draft.fields.length >= 64" @click="addField">
            添加字段
          </button>
        </header>
        ...
        >
          <header class="template-field-card__header">
            ...
            >
              ...
              <span class="template-field-card__identity">
                ...
                <span>{{ fieldTypeLabel(field.fieldType) }}{{ field.required ? ' · 必填' : '' }}</span>
              </span>
              <svg viewBox="0 0 16 16" aria-hidden="true"><path :d="field.expanded ? 'm4 10 4-4 4 4' : 'm4 6 4 4 4-4'" /></svg>
            </button>
            <span class="template-field-card__actions">
              <button class="icon-button" type="button" title="上移字段" :aria-label="`上移字段 ${field.fieldName || index + 1}`" :disabled="submitting || index === 0" @click="moveField(index, -1)">↑</button>
              <button class="icon-button" type="button" title="下移字段" :aria-label="`下移字段 ${field.fieldName || index + 1}`" :disabled="submitting || index === draft.fields.length - 1" @click="moveField(index, 1)">↓</button>
              <button class="icon-button template-field-card__delete" type="button" title="删除字段" :aria-label="`删除字段 ${field.fieldName || index + 1}`" :disabled="submitting || draft.fields.length === 1" @click="requestDeleteField(index)">×</button>
            </span>
          </header>
          ...
          <div v-show="field.expanded" :id="`${field.key}-body`" class="template-field-card__body">
            ...
            <div class="template-field-card__switches">
              <label><input v-model="field.required" type="checkbox" :disabled="submitting" /> <span>实际录入必填</span></label>
              <label><input v-model="field.searchable" type="checkbox" :disabled="submitting" /> <span>允许参与筛选</span></label>
            ...
            <div v-if="field.fieldType === 'file'" class="template-field-card__hint">图片字段不支持默认文件。</div>
            <FormSelect
            ...
            >
              <option value="">不设置</option>
              <option v-for="option in normalizedOptions(field.options)" :key="option" :value="option">{{ option }}</option>
            </FormSelect>
            <FormInput
              v-else
              v-model="field.defaultValue"
              ...
              :error="errors[`fields.${index}.default_value`]"
              maxlength="256"
              :placeholder="field.fieldType === 'date' ? 'YYYY-MM-DD' : field.fieldType === 'url' ? 'https://example.com' : undefined"
              autocomplete="off"
      ...
      <p v-if="errorMessage" class="form-error template-editor__request-error" role="alert">{{ errorMessage }}</p>
    </form>
    ...
    <div v-else class="template-detail">
      ...
      <section class="template-detail__fields" aria-label="字段结构">
        ...
        <article v-for="(field, index) in draft.fields" :key="field.key">
          <header><span>{{ index + 1 }}</span><strong>{{ field.fieldName }}</strong><em>{{ fieldTypeLabel(field.fieldType) }}</em></header>
          <dl>
            <div><dt>录入规则</dt><dd>{{ field.required ? '必填' : '选填' }} · {{ field.searchable ? '可筛选' : '不参与筛选' }}</dd></div>
            <div v-if="field.defaultValue"><dt>默认值</dt><dd>{{ field.defaultValue }}</dd></div>
            <div v-if="field.options.length"><dt>候选项</dt><dd>{{ field.options.join('、') }}</dd></div>
            <div v-if="kind === 'item'"><dt>单位</dt><dd>{{ unitLabel(field) }}</dd></div>
            <div v-if="kind === 'item'"><dt>目录展示</dt><dd>{{ field.catalogVisible ? '是' : '否' }}</dd></div>
          </dl>
        </article>
      </section>
    </div>
    ...
    <template #actions>
      <button class="secondary-button" type="button" :disabled="submitting" @click="requestClose">{{ readOnly ? '关闭' : '取消' }}</button>
      <button v-if="readOnly && canEdit" class="primary-button" type="button" @click="emit('edit')">编辑模板</button>
      <button v-else-if="!readOnly" class="primary-button" type="submit" :form="formId" :disabled="submitting">
  ...
  >
    <template #actions>
      ...
      <button :class="confirmState?.danger ? 'danger-button' : 'primary-button'" type="button" @click="runConfirmedAction">
...
import type { ItemAttributeTemplateResponse, ItemAttributeUnitMode } from '../../api/itemAttributeTemplates'
...
const dialogTitle = computed(() => props.readOnly
  ? `${props.kind === 'item' ? '物品属性' : '入库'}模板详情`
  : props.template ? `编辑${props.kind === 'item' ? '物品属性' : '入库'}模板` : `新建${props.kind === 'item' ? '物品属性' : '入库'}模板`)
const dialogDescription = computed(() => props.kind === 'item'
  ? '定义物品长期属性、单位和最多三个目录展示字段。'
  : '定义单次收货时记录的动态属性字段。')
const catalogVisibleCount = computed(() => draft.fields.filter((field) => field.catalogVisible).length)
const unresolvedDefaultInbound = computed(() => props.kind === 'item' && draft.defaultInboundTemplateId !== null &&
...
const readonlyDefaultInboundLabel = computed(() => {
  ...
  return props.inboundTemplates.find((item) => item.id === draft.defaultInboundTemplateId)?.name
    ?? `已删除入库模板 #${draft.defaultInboundTemplateId}`
})
...
function addField(): void {
  ...
  void nextTick(() => document.querySelector<HTMLElement>(`#${draft.fields.at(-1)?.key}-body input`)?.focus())
}
...
function requestDeleteField(index: number): void {
  ...
  if (!field.fieldName.trim() && !field.defaultValue.trim() && !field.options.some((item) => item.trim())) {
    remove()
    return
  }
  confirmState.value = {
...
function requestFieldTypeChange(index: number, value: unknown): void {
  ...
  const needsConfirmation = Boolean(field.defaultValue.trim() || field.options.some((option) => option.trim()))
  const apply = () => clearIncompatibleFieldData(field, nextType)
  if (!needsConfirmation) apply()
  else confirmState.value = {
    title: '更改字段类型？',
...
function requestUnitModeChange(index: number, value: unknown): void {
  ...
  const hasCurrentData = field.unitValue.trim() || field.unitOptions.some((option) => option.trim())
  const apply = () => {
