...
<template>
  ...
  >
    <form id="item-catalog-filter-form" class="item-catalog-filter" @submit.prevent="apply">
      ...
      <div v-else-if="error" class="item-catalog-filter__state item-catalog-filter__state--error" role="alert">
        <span>{{ error }}</span>
      ...
      <div v-else-if="visibleFields.length" class="item-catalog-filter__groups">
        <section v-for="field in visibleFields" :key="field.key" class="item-catalog-filter__group">
          <header>
            ...
            <span>{{ selectedValues(field.key).length ? `已选 ${selectedValues(field.key).length}` : `${field.values.length} 项` }}</span>
          </header>
          <div class="item-catalog-filter__values">
            <label
              ...
              :class="{ 'is-selected': selectedValues(field.key).includes(value.value), 'is-unavailable': value.unavailable }"
            >
              <input
                type="checkbox"
              ...
              />
              <span :title="displayValue(field, value.value)">{{ displayValue(field, value.value) }}</span>
          ...
          >
            {{ expandedFields.includes(field.key) ? '收起' : `显示其余 ${mergedValues(field).length - COLLAPSED_VALUE_COUNT} 项` }}
          </button>
        </section>
      </div>
    ...
    <template #actions>
      <button class="text-button item-catalog-filter__clear" type="button" :disabled="!hasDraftFilters" @click="clearDraft">清除全部</button>
...
const hasDraftFilters = computed(() => draft.categoryId !== null
...
function displayValue(field: ItemFilterFieldResponse, value: string): string {
  if (field.value_type === 'boolean') return value === 'true' ? '是' : value === 'false' ? '否' : value
  return value
}
...
.item-catalog-filter { display: grid; gap: 18px; }
.item-catalog-filter__fixed-fields { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; }
.item-catalog-filter__fixed-fields label { display: grid; min-width: 0; gap: 6px; color: var(--color-muted); font-size: 12px; font-weight: 650; }
.item-catalog-filter__groups { display: grid; gap: 16px; }
.item-catalog-filter__group { display: grid; gap: 8px; padding-top: 14px; border-top: 1px solid var(--color-border); }
.item-catalog-filter__group > header { display: flex; min-width: 0; align-items: baseline; justify-content: space-between; gap: 12px; }
.item-catalog-filter__group > header strong { overflow: hidden; font-size: 13px; text-overflow: ellipsis; white-space: nowrap; }
.item-catalog-filter__group > header span { color: var(--color-subtle); font-size: 11px; white-space: nowrap; }
.item-catalog-filter__values { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 6px; }
.item-catalog-filter__value {
  display: grid; min-width: 0; min-height: 36px; grid-template-columns: 16px minmax(0, 1fr) auto; align-items: center; gap: 7px;
  padding: 7px 8px; border: 1px solid var(--color-border); border-radius: var(--radius-sm); color: var(--color-text); cursor: pointer;
}
.item-catalog-filter__value.is-selected { border-color: color-mix(in srgb, var(--color-accent) 45%, var(--color-border)); background: var(--color-accent-soft); color: var(--color-accent); }
.item-catalog-filter__value.is-unavailable { border-style: dashed; color: var(--color-muted); }
.item-catalog-filter__value input { width: 15px; height: 15px; margin: 0; accent-color: var(--color-accent); }
.item-catalog-filter__value > span { overflow: hidden; font-size: 12px; font-weight: 620; text-overflow: ellipsis; white-space: nowrap; }
.item-catalog-filter__value small { color: var(--color-subtle); font-size: 11px; font-weight: 550; }
.item-catalog-filter__expand { justify-self: start; }
.item-catalog-filter__state { display: flex; min-height: 84px; align-items: center; justify-content: center; gap: 8px; color: var(--color-muted); font-size: 12px; text-align: center; }
.item-catalog-filter__state--error { color: var(--color-danger); }
.item-catalog-filter__clear { margin-right: auto; }

@media (max-width: 767px) {
  .item-catalog-filter__fixed-fields,
  .item-catalog-filter__values { grid-template-columns: minmax(0, 1fr); }
}
</style>
