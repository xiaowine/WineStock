...
export function useItemCreateSession() {
  ...
  async function save(): Promise<ItemOptionResponse | null> {
    ...
    try {
      ...
      if (!created) throw new Error("The newly created item did not appear in the item picker");
...
function itemCreateErrorMessage(error: unknown): string {
  return error instanceof ApiError ? error.message : translateMessageOrNull("error.network_unavailable") ?? "";
}
