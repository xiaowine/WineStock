...
function positionVerticalThumb(
  ...
  const trackLength =
    rect.height - TRACK_INSET * 2 - (entry.horizontalThumb ? THUMB_HIT_SIZE : 0)
  ...
  const thumbLength = Math.min(
    trackLength,
    Math.max(MIN_THUMB_LENGTH, trackLength * (entry.element.clientHeight / entry.element.scrollHeight)),
  )
...
function positionHorizontalThumb(
  ...
  const trackLength =
    rect.width - TRACK_INSET * 2 - (entry.verticalThumb ? THUMB_HIT_SIZE : 0)
  ...
  const thumbLength = Math.min(
    trackLength,
    Math.max(MIN_THUMB_LENGTH, trackLength * (entry.element.clientWidth / entry.element.scrollWidth)),
  )
  const travel = Math.max(0, trackLength - thumbLength)
