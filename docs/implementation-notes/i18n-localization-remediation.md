...
- 验证：`pnpm build`（vue-tsc + vite）✅；前端 node 测试 86 通过 0 失败（3 轮稳定）✅；Rust workspace 216 测试全过 + `check --all-targets` 无警告 ✅；i18n 运行时冒烟（语言决议/键对齐/翻译取样）✅；**desktop WebView2 CDP 真机 UI 冒烟**（`pnpm desktop:dev` + 127.0.0.1:9222）✅——zh 默认渲染、偏好设置语言切换、zh↔en 全站文案/`document.title`/`lang` 即时切换、localStorage 持久化、用户/看板/物品页双语言渲染、移动视口 390px 无横向溢出、控制台无 error/warning；冒烟中发现并修复 3 处真问题（导航裸键渲染、物品筛选中文括号残留、纯逻辑模块 i18n 导入约束）。**Android 真机验证**（Redmi K40 / Android 13，重新构建 debug APK 后安装）✅——应用全链路启动（WebView + Shell Bridge + 本地 core + 会话恢复）、中文默认渲染、偏好设置切换 English 全站英文并高亮选中、切回中文恢复；APK 内容确认 Kotlin 字段错误码化、Rust native 码、values-en 英文资源、前端 i18n 均已打包。**Android 复杂测试**（专用测试机）✅——① 系统语言 en-US + 清前端 localStorage 偏好后首启自动英文（`navigator.language` 决议分支）；② 强制 WebView 兼容页（Debug extra `FORCE_WEBVIEW_BLOCK`）在英文系统下全英文（values-en 运行时）、诊断码 `WEBVIEW_VERSION_TOO_OLD` 不译；③ 切回中文系统对照同一兼容页全中文（values 默认）；④ 仅清 WebView localStorage（`run-as` 删 `app_webview/Default/Local Storage`），本地 core 数据库与业务数据保留。环境要点：MIUI 切系统语言必须走系统设置 UI（权威源为 `persist.sys.locale`，`settings put system_locales` 无效）。`pnpm format:check` 因本机 git autocrlf=true 导致全仓 CRLF 与 prettier `endOfLine: lf` 冲突而失败（未改动的 `vite.config.ts` 同样失败，属既有环境问题，未做全仓 EOL 重写以避免无关 diff）。
