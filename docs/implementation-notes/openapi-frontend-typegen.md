...
- Release 边界：纯契约类型的 controller 重新导出按使用方门控。仅被 Debug OpenAPI 注册引用的 `FileAttributeReference`/`ItemAttributeValue` 使用 `#[cfg(debug_assertions)]`；被现有 Rust 测试经 controller 门面导入的 `InboundItemRequest`/`OutboundItemRequest`/`SubstituteReplacementItem` 保留 `#[cfg(any(test, debug_assertions))]`。Release 运行时这些类型都仅由父请求结构携带，Release 构建保持零未使用导入告警。
