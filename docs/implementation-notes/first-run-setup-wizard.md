...
当前状态：**已实施**（`frontend/src/pages/SetupWizardPage.vue`、共享主题选择器、`telemetry/consent.ts`、
...
2. 无法承载"首次体验"应有的欢迎语义与本机偏好（主题、数据收集同意）。
...
| 3 | 偏好设置 | 主题三态分段控件 + 匿名使用数据开关与隐私声明入口 | 主题跟随系统；数据收集开（可取消） |
...
- 数据收集开关说明：帮助开发者定位和排查问题；不包含库存内容与账户信息，仅在联网时生效。
  ...
  默认开启，用户可在完成前取消，也可随时在「偏好设置」中更改。
- 主题选择复用偏好设置 Dialog 的三态控件，顺序固定为「跟随系统 / 浅色 / 深色」；选择后立即应用并持久化，
  不等待 Shell 运行配置 apply，也不增加单独步骤。
...
主题不新增向导页面，而是与数据收集合并为最后一页本机偏好；首次默认跟随系统，用户可以在进入业务界面前
手动选择浅色或深色。向导与后续偏好设置必须复用同一个主题选择组件，不能复制状态或样式。
语言仍跟随系统语言；将来多语言落地时在欢迎页角落放当前语言切换器。向导总页数继续保持 3 页上限。
...
- 纯网页端（快照 `platform === "web"`）：只有「连接已有服务器」一种能力，使用方式页
  没有可做的决策，**整页跳过**——向导直接从服务器地址页开始（服务器地址 → 偏好设置，
...
- 主题默认跟随系统，三态选择立即生效并持久化；向导与偏好设置 Dialog 的选中状态保持一致；
- 数据收集默认开且可取消；完成时按最终选择持久化，Network 面板确认取消时无 Clarity 请求
...
- 不在向导中增加独立主题页或自定义配色；语言切换仍不在本次范围。
