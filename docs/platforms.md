...
- Desktop 主窗口支持在偏好设置中选择“最小化到系统托盘”或“退出应用”；默认最小化到托盘。托盘隐藏默认只隐藏窗口，
  不停止本地 core；开启 Desktop WebView 回收后，隐藏超过用户设置的时长才销毁主 WebView，Tauri 进程、托盘和本地 Axum
  继续运行，托盘恢复时重新加载前端。托盘“退出”以及选择直接退出后的窗口关闭，统一等待本地 Axum 优雅停止。托盘不可用时采用直接退出的安全降级，
  不把窗口隐藏到无法恢复的状态；
