...
- `frontend/src/components/attributes/`：物品与入库共用的单张图片属性控件、无依赖 HSV 颜色选择器、鉴权图片加载和图片草稿/纯色生成/提交阶段批量上传模型。
...
- 入库：`InboundDraftPage` 单物品串行工作台（版本化草稿 + IndexedDB 图片）、`InboundOrdersPage` 单据列表，`components/inbound/` 明细编辑器、复核区、筛选与列表件。
...
- 模板：`TemplatesPage` 分类/物品属性模板/入库模板三资源工作区，`components/templates/` 字段编辑、复制与三类差异化删除确认。
...
- `frontend/src/storage/`：入库草稿本地图片 Blob 的 IndexedDB 存取，不保存普通字段或执行上传。
