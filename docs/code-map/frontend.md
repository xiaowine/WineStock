...
- 本机静默会话：Shell 快照携带 `localAuthExchangeToken`（仅 ownership=local）时进入静默免登录模式——refresh 无法恢复会话则用换取凭据调 `/api/auth/local-session` 静默建会话；服务端返回 `local_session_unavailable` 或空库首换取所需的 `local_initial_user_required` 时按匿名状态进入 `/auth`，self-hosted 空库复用 `/register` 的本机初始化变体，其余失败落 `unavailable` 并入服务可用性覆盖层。静默模式下 AppShell 显示当前用户名、隐藏退出登录，并收起用户管理及出入库审批一级导航；审批路由仍供历史或异常待审批单据从详情页进入。设计见 `docs/implementation-notes/self-hosted-silent-auth.md`。
...
- 立创建档：`components/items/` 统一把 Core 选定的商品图转换为主图草稿并在缺失/读取失败时生成占位图；批量创建会话串行执行查资料→拉图→套批次模板→创建，保持单项失败不阻塞、`sku_taken` 自动匹配和中止安全，供订单导入与备份导入共用。
- ERP 备份导入：`src/erp/backupImport.ts` 解析纯逻辑（第三方「LCSC Android ERP」xlsx 四表按表头解析并 join、C0 手工器件分流、double→int、schema 版本门，node 单测覆盖）与 `backupImportFile.ts` 动态 SheetJS 读取入口；`components/stock-draft/ErpBackupImportDialog.vue` 编排预览（库位/器件勾选批量创建/C0 跳过）与执行（新建缺失库位→批量创建→组装期初库存行），经 `useInboundDraft.importBackup` 写入草稿并预填来源/备注。实施见 `docs/implementation-notes/erp-backup-import-implementation.md`。
...
- 用户：`UsersPage` 拥有 HTTP 列表、筛选、分页、权限判断和 Dialog 编排；`components/users/UserDirectoryTable` 与 `UserDirectoryMobileList` 共享用户数据分别负责桌面直接列和移动纵向列表，`UserStatusPill`、`UserPasswordStatePill`、`userDirectory.ts` 复用状态/日期/展示动作规则；用户管理表单族包含独立用户名修改 Dialog，编辑当前账号时锁定权限管理关键权限，改名后同步当前会话用户快照。
