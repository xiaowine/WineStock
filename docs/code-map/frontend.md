...
- `frontend/src/main.ts`：注册共享 API client 的有效 token provider、异步恢复持久会话、注册 Vue Router 并挂载根组件。
...
- `frontend/src/layouts/DesktopShell.vue`：桌面顶部状态栏、左侧导航、当前用户摘要和嵌套路由出口。
...
- `frontend/src/api/auth.ts`
  - 定义注册、登录、refresh、用户摘要和 token 响应 DTO。
  - 当前实现 `POST /api/auth/register`、`POST /api/auth/login` 和 `POST /api/auth/refresh`。

- `frontend/src/auth/session.ts`
  - 在内存保存 access token、预计过期时间和用户摘要；refresh token 只从统一 localStorage 读取。
  - 保留同标签页 refresh Promise，并在跨标签页锁内读取最新 refresh token、轮换和恢复会话。
  - 监听其它同源标签页移除持久 token，并同步清除当前内存会话。

- `frontend/src/auth/coordination.ts`
  - 使用 Web Locks API 在同源标签页和 Worker 间串行执行 refresh token 轮换。
...
- `frontend/src/pages/login/DesktopLoginPage.vue`：桌面用户名密码表单，调用登录 API、映射字段错误并在成功后建立内存会话。
