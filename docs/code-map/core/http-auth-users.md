...
- `core/src/http/docs.rs`：OpenAPI 路径、元信息、业务 tag 和 Swagger UI。
...
- `core/src/auth/mod.rs`：注册 login、refresh 和 logout 路由。
- `core/src/auth/contract.rs`：登录、刷新、登出、用户摘要、token 响应和客户端类型 DTO。
