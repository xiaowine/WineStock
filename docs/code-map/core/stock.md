...
- `mod.rs`：以 `/api` 为 base path 注册各子域路由，并在装配阶段声明权限。
...
- 物品（`items`、`item_attributes`）：命令与查询分离、必选主图、目录结构化筛选解析与定义校验、轻量选择、库存详情、批次分页和任意类型化属性校验。
- 立创查询（`item_lookup`）：客编校验、精确结果选择、已知字段投影、库存/第一阶梯价判断、受控商品图地址投影和稳定外部错误映射；不写数据库，不公开上游原始响应，图片内容读取与复核由前端 `lcsc/image.ts` 拥有。
- 模板（`templates/`）：分类与物品属性模板的独立 CRUD/复制、当前有效物品使用数投影、删除影响数和物品模板单位规则归一化。
- 库位（`locations`）：最多十层分组树的创建/子树移动、名称唯一且可带备注的库位 CRUD、整批次移库与循环/占用校验。
- 入库（`inbound`）：提交模式、明细物品与库位有效性校验、直接入库权限判断、审批与错误定位。
...
- `service/pagination.rs`、`response.rs`、`validation.rs`：库存通用分页、repository 记录到 HTTP DTO 的投影和文本/数值/ID/JSON 归一化。

## 启动补齐

- `bootstrap/`：幂等补齐三个分类和三套物品属性预设；只在没有任何有效库位时创建 `默认库区`/`默认库位`，不覆盖用户修改，不恢复被软删除的模板。
