...
- 文件域：文件元数据，以及物品/入库属性行与文件对象的一对一绑定；物品主图由 `stock_items.image_file_id` 直接引用。
- 库存域：物品、分类、实际属性、可选物品属性预设（字段级显式单位规则）、入库模板与实际入库属性。
...
- 子域查询模块与库存业务子域一一对应：物品（命令、目录实时库存聚合、参数化筛选与状态计数、批次分页）、分类与模板（使用数聚合、删除事务内影响数）、库位（分组、整批次移库）、入库（含状态条件分页、事务内逐行属性与图片绑定，直接入库与审批复用同一事务步骤）、出库（状态筛选、指定批次或 FIFO 扣减）、看板聚合、替代料关系、审计事件查询。
