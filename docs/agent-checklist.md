...
- 地图粒度遵守 `docs/code-map.md` 的粒度约定：条目单位是模块/目录/领域，逐文件职责由源码中文文件头注释承担。
- 模块或领域的职责、边界或依赖方向变化，以及新增/移动模块、crate、页面域或共享组件域时，在同一改动中更新对应子地图；在既有模块内新增文件且文件头注释已说明职责的，不要求新增地图条目。
- 只有新增顶层组件、增加/删除地图文件、改变顶层依赖方向或调整粒度约定时，才更新 `docs/code-map.md` 根索引。
...
1. Debug 服务暴露 `/api-docs/openapi.json` 时，优先读取当前运行服务输出；它是当前公开 HTTP 契约的直接证据。
...
Debug server builds include Swagger UI by default; Release server builds expose neither endpoint.
