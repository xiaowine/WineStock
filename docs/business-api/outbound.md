...
- 响应明细会返回 `location_id`、`location_code` 和 `location_name`；未指定库位时这些字段为空。
...
- 说明：出库单搜索会匹配出库去向、备注、状态、库位编码/名称、关联物品基础字段；对已指定批次或已审批写入流水的明细，还会匹配批次号、有效期和入库模板实际值。结果按出库单去重。空 `search` 返回 `400 invalid_request`。
