...
Desktop 主窗口点击关闭后不退出进程，而是隐藏窗口并继续在系统托盘运行；用户通过托盘图标或托盘菜单重新打开主窗口。开启独立的 WebView 空闲回收偏好后，隐藏超过设置时长会销毁主 WebView，但进程、托盘和本地服务仍继续运行。只有用户选择托盘菜单中的“退出”时，应用才执行现有的本地服务优雅停止流程并结束进程。
